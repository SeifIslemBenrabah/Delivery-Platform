...
const getProduitBySearch = async (req, res) => {
  try {
    ...
    if (produits.length === 0) {
      return res
        .status(404)
        .json({ msg: "There is no product with this name." });
    }
...
const getProduitByIdCatalogue = async (req, res) => {
  try {
    ...
    const boutique = await Boutique.findById(
    ...
    const catalogue = boutique.catalogues.find(
      (cat) => cat._id.toString() === catalogueId
    );
    ...
    if (produits.length === 0) {
      return res
        .status(200)
        .json({ message: "No products found in this catalog." });
    }
...
const updateProduit = async (req, res) => {
  try {
    ...
    if (
      updateData.Boutiqueid &&
      !mongoose.Types.ObjectId.isValid(updateData.Boutiqueid)
    ) {
    ...
    if (updateData.infos) {
      try {
        ...
        return res
          .status(400)
          .json({ message: "Invalid JSON format in infos" });
      }
    }
    if (req.file) {
